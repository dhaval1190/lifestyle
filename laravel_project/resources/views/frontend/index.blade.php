            ...
            <div class="row mt-5 mb-5">
                ...
                <div class="row mb-4">
                        <div class="col-md-7 text-left border-primary">
                            <h2 class="font-weight-light text-primary">{{ __('Featured Coaches') }}</h2>
                        </div>
                ...
                <div class="swiper-container">
                    ...
                    <div class="swiper-wrapper">
                        ...
                        @foreach($all_trainding_items as $trainding_items_key => $trainding_item)
                            <div class="swiper-slide">
                                <div class="listing">
                                        <a href="{{ route('page.item', $trainding_item->item_slug) }}" class="img d-block" style="background-image: url({{ !empty($trainding_item->item_image_medium) ? Storage::disk('public')->url('item/' . $trainding_item->item_image_medium) : (!empty($trainding_item->item_image) ? Storage::disk('public')->url('item/' . $trainding_item->item_image) : asset('frontend/images/placeholder/full_item_feature_image_medium.webp')) }})"></a>
                                        <div class="lh-content">
                                                ...
                                                <h3 class="pt-2" style="word-break: break-all;"><a href="{{ route('page.item', $trainding_item->item_slug) }}">{{ $trainding_item->item_title }}</a></h3>

                                                @if($trainding_item->item_type == \App\Item::ITEM_TYPE_REGULAR)
                                                <address>
                                                    <a href="{{ route('page.city', ['state_slug'=>$trainding_item->state->state_slug, 'city_slug'=>$trainding_item->city->city_slug]) }}">{{ $trainding_item->city->city_name }}</a>,
                                                    <a href="{{ route('page.state', ['state_slug'=>$trainding_item->state->state_slug]) }}">{{ $trainding_item->state->state_name }}</a>
                                                </address>
                                                @endif
                                                ...
                                                @if($latest_item_getCountRating > 0)
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <div class="pl-0 rating_stars rating_stars_{{ $trainding_item->item_slug }}" data-id="rating_stars_{{ $trainding_item->item_slug }}" data-rating="{{ $trainding_item->item_average_rating }}"></div>
                                                            <address class="mt-1">
                                            ...
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-12 col-md-7 pr-0">
                                                    <div class="row align-items-center item-box-user-div">
                                                        ...
                                                        <a class="decoration-none" href="{{ route('page.profile', encrypt($trainding_item->user->id)) }}">
                                                            @if(empty($item->user->user_image))
                                                                <img src="{{ asset('frontend/images/placeholder/profile-'. intval($trainding_item->user->id % 10) . '.webp') }}" alt="Image" class="img-fluid rounded-circle">
                                                            @else
                                                                <img src="{{ Storage::disk('public')->url('user/' . $trainding_item->user->user_image) }}" alt="{{ $trainding_item->user->name }}" class="img-fluid rounded-circle">
                                                            @endif
                                                        </a>
                                                        </div>
                                                        <div class="col-9 line-height-1-2 item-box-user-name-div">
                                                            <div class="row pb-1">
                                                                <div class="col-12">
                                                                    ...
                                                                    <a class="decoration-none" href="{{ route('page.profile', encrypt($trainding_item->user->id)) }}"><span class="font-size-13">{{ $trainding_item->user->name }}</span></a>
                                                                </div>
                                                            </div>
                                                            <div class="row line-height-1-0">
                                                                <div class="col-12">
                                                                    <span class="review">{{ $trainding_item->created_at->diffForHumans() }}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
