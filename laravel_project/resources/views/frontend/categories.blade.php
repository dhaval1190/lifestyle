...
@section('content')
    ...
    <div class="container">
        <div class="row align-items-center justify-content-center text-center">
            <div class="col-md-10 mb-5 pb-3" data-aos="fade-up" data-aos-delay="400">
                    ...
                    @if ($paid_items->count() <= 4)
                        <div class="row ">                            
                            @foreach ($paid_items as $paid_items_key => $item)
                                <div class="col-lg-3">
                                    @include('frontend.partials.paid-item-block-simple')
                                </div>                                    
                            @endforeach                            
                        </div>
                    @endif
                ...
                <div class="col-lg-12 block-13">
                    ...
                    @if ($free_items->count() <= 4)
                        <div class="row">
                            @foreach ($free_items as $free_items_key => $item)
                            <div class="col-lg-3">
                                @include('frontend.partials.free-item-block-simple')
                            </div>
                            @endforeach
                        </div>
                    @endif
                ...
                @if (count($paid_items) || count($free_items))
                    ...
                    @foreach ($paid_items as $paid_items_key => $paid_item)
                        @if ($paid_item->item_type == \App\Item::ITEM_TYPE_REGULAR)
                            ...
                            var marker = L.marker([{{ $paid_item->item_lat }}, {{ $paid_item->item_lng }}]).addTo(
                                map);
...
@section('carousel_scripts')
    ...
    @if ($free_items->count() == 5 || $free_items->count() == 6)
        <script>
            // alert("dslkdlk");
