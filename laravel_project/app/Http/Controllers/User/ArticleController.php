...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return response()->view('backend.user.article.index',
    ...
    public function create(Request $request)
    {
        ...
        // echo "<pre>";
        // print_r($items->itemHours);exit;
        $items_hours = isset($items) && count($items->itemHours) > 0 ? $items->itemHours[count($items->itemHours)-1] : [];
        $items_hours_exceptions = isset($items) && count($items->itemHourExceptions) > 0 ? $items->itemHourExceptions[count($items->itemHourExceptions)-1] : [];
            ...
            foreach($item_hour_exceptions as $item_hour_exceptions_key => $item_hour_exception)
            {
                ...
                if(DateTime::createFromFormat('Y-m-d', $item_hour_exception_date) !== false)
                {
                    ...
                    $create_item_hour_exception->save();
                        ...
                        $data["subject"] = 'New Topic ('.$item_title.') Create At: ' .$data["date"].' ('.$data["article_by"].')';
                        /**
    ...
    public function savedItems()
    {
        ...
        SEOMeta::setTitle(__('seo.backend.user.article.saved-items', ['site_name' => empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name]));
        SEOMeta::setDescription('');
        SEOMeta::setCanonical(URL::current());
        SEOMeta::addKeyword($settings->setting_site_seo_home_keywords);
        /**
        ...
        return response()->view('backend.user.article.saved',
            compact('saved_items'));
    }
    ...
    public function itemReviewsCreate(string $item_slug)
    {
        ...
        if($item)
        {
            ...
            {
                return response()->view('backend.user.article.review.create',
                    compact('item'));
            }
    ...
    public function itemReviewsEdit(string $item_slug, int $review)
    {
        ...
        if($item)
        {
            if($item->hasReviewByIdAndUser($review, Auth::user()->id))
            {
                ...
                return response()->view('backend.user.article.review.edit',
    ...
    public function itemReviewsIndex(Request $request)
    {
        ...
        return response()->view('backend.user.article.review.index',
    ...
    public function indexItemSections(Request $request, Item $item)
    {
        ...
        return response()->view('backend.user.article.item-section.index',
    ...
    public function editItemSection(Request $request, Item $item, ItemSection $item_section)
    {
        ...
        else
        {
            ...
            return response()->view('backend.user.article.item-section.edit',
...
// dd($request->all());
        ...
        if($item && $item->user_id == $login_user->id)
        {
            ...
            if(!is_null($item_hour_exception_open_time_hour) && !is_null($item_hour_exception_open_time_minute) && !is_null($item_hour_exception_close_time_hour) && !is_null($item_hour_exception_close_time_minute))
            {
